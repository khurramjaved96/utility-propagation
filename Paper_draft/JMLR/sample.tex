...
{\stepcounter{todocounter}
	\todo[color=green!40,author=Khurram, inline]{\thetodocounter: #1}
}

\newcommand{\khurram}[1]{\stepcounter{todocounter}
	{\color{blue!90} Khurram: \thetodocounter: #1}}
...
\subsection{Animal Learning Benchmarks}\label{animal_bench}
A suitable benchmark that fits this problem formulation is the trace patterning benchmark introduced by \cite{Rafiee2020TestbedsFR}. In this benchmark, we are provided with a set of binary stimuli $o_t \in \R^d$ at each timestep t, which includes a set of $n$ Conditioned Stimuli CS$_t $, one Unconditioned Stimuli US$_t$ and $m$ distractor signals. There are $k$ random sets of  configurations of CSs that will activate the US. If the active values of CS$_t$ match one of these activation patterns, the value US$_{t+\text{ISI}}$ will be set to 1, where ISI (\textit{inter-stimulus interval}) is a problem hyperparameter that is uniformly sampled from a certain range after every occurence of the US. In this problem, the goal is to learn eq. \ref{eq:return} using TD error where 
\begin{equation}
	y(k) = \text{US}_{t+k+1}
\end{equation}
...
\begin{figure*}[t]
	\centering
	% More versions of the left figure available in the figures folder
	\includegraphics[width=0.48\textwidth]{figures/plt_animal_preds400_tight_withaxlabel.eps}
	\includegraphics[width=0.48\textwidth]{figures/plt_animal.eps}
	\label{animal_results}
	\caption{ Left: Comparison of the predictions produced by the different methods for the last 400 timesteps of training. Right: Performance when an LSTM is trained using the proposed and other online baseline methods on the trace patterning benchmark (lower is better). All the runs are averaged using 30 seeds, and the error regions are the 95\% confidence intervals. T-BPTT uses a truncation length k=27. We can see from this plot that the Diagonal Jacobian method performs the worst while the hybrid networks achieve the lowest error. TODO: T-BPTT is only 15 runs while the remaining curves are 30 runs. TODO: Maybe hybrid networks should get a better name so that people can refer to it easily?}
\end{figure*}
We first evaluate the performance of the proposed methods and baselines on the trace patterning benchmark described in section \ref{animal_bench}. In these experiments, we set $\text{ISI} = \text{Unif}(14,26)$, $\g = \frac{1}{1-\E[ISI]}$, $n=6$ CSs, $k=10$ activation patterns with a $0.5$ probability of activation and $m=5$ distractor signals. The details of the remaining hyper-parameters and their tuning is given in the Appendix \ref{hyperparams}.
...
\khurram{They are in khurram\_constructive\_nomralized\_2. Use the features column to select the size of the network}
...
We performed an exhaustive search over all the hyper-parameter ranges for each of the methods that is presented in this paper. For each hyper-parameter configuration, we averaged its performance over at-least three runs and determined the best setting based on the lowest average final error. The sweep ranges for the different hyper-parameters are provided in Table \ref{tab:sweep-ranges} and the best hyper-parameters are provided in Table \ref{tab:best-params}.
...
\begin{table}[h]
	...
	\begin{tabular}{@{}cllcccc@{}}
		\toprule
		 &  Hyper-parameter & Sweep Ranges\\ \midrule
		$\a$  & Step-size     & $\{3^{-1}, 1^{-1}, 3^{-2}, 1^{-2}, 3^{-3}, 3^{-4}\}$ \\
		$\l$ & Eligibility Traces Decay Rate     & $\{0, 0.5, 0.9, 0.99\}$               \\
	...
	\end{tabular}
	\vspace{0.15cm}
	\caption{Sweep ranges for each hyper-parameter. TODO: The notation for last two rows is just placeholder. Replace it with something appropriate.}
