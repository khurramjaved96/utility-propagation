...
//
  ...
  auto network = RecurrentNetworkMultilayer(my_experiment.get_float_param("step_size"),
                                  my_experiment.get_int_param("seed"),
                                  28,
                                  10,
                                  my_experiment.get_int_param("features"),
                                  my_experiment.get_int_param("layers"),
...
//      std::cout << "Row = " << row << std::endl;
      ...
      if(row != 27) {
        network.backward(network.read_output_values(), layer);
        network.update_parameters();
      }
      else{

        network.backward(y, layer);
...
//    print_vector(network.read_output_values());
//    if(i%100 == 0) {
//      for (auto syn : network.output_synapses) {
//        std::cout << syn->input_neuron->layer_number << " " << syn->weight << std::endl;
//      }
//    }
